...
Hover a Codex or Claude ring. Codex shows only its main weekly quota (whether the API calls it primary or secondary). Claude starts with Fable Weekly, then the five-hour window, then All Models Weekly. Other available Claude windows follow. Each chart has its own hover cursor and 15-minute controls. It opens focused on recorded history (up to the last six hours), with a small future margin. Click **History ↔ / Window ↔** to switch between this readable view and the entire quota window. The target always represents the original whole-cycle pace; zooming does not rebase it. The synthetic Claude Daily pace ring is not a quota cycle and is excluded from the charts; the ring setting remains available.
