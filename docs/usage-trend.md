...
Hover a Codex or Claude ring. Codex shows only its main weekly quota (whether the API calls it primary or secondary). Claude starts with Fable Weekly, then the five-hour window, then All Models Weekly. Other available Claude windows follow. Each chart has its own hover cursor and 15-minute controls. The chart always shows the entire quota cycle: the whole week for weekly windows, and all five hours for the session window. There is no automatic zoom or range toggle. Short recorded histories occupy only their real part of that fixed time axis. The synthetic Claude Daily pace ring is not a quota cycle and is excluded from the charts; the ring setting remains available.
...
- **Percentages:** Codex and Claude charts both show **used** quota, matching their rings. The same conversion applies to the header, hover values, target and forecast. Stored readings remain in their original remaining-quota format.
- **Target (dashed):** a constant pace from 0% used at the start (`reset - duration`) to 100% at reset.
- **Actual (accent):** locally observed used quota, with real observation timestamps. History starts when this build begins collecting it; no historical zeroes or synthetic backfill.
- **Forecast (dashed accent):** extends the latest reading using the average consumption over the entire current quota window: `(1 - remaining fraction) / (observation time - window start)`. This includes pauses and time before local recording began. Local gaps, sample density and short bursts do not change this basis. Each weekly or five-hour window uses its own start and cumulative usage, labelled **Forecast: average since window start**. It stops at exhaustion or reset. **Lasts for** estimates the remaining time or states that the allowance lasts until reset under the same workload. A recent flat segment does not erase consumption earlier in the window. Stale readings do not produce a forecast.
...
Only windows with a published duration, reset and percentage offer the chart. Reset timestamps drifting by up to five seconds are matched within the same provider, window and account. This also recovers previously recorded Claude observations fragmented by subsecond reset jitter. The first and last real reading of every 15-minute bucket are retained, so a visible actual segment does not have to wait for the next bucket. History before collection began cannot be reconstructed. Readings are kept locally in UserDefaults, bounded to 120 days and 12,000 samples across providers. Empty clock buckets break the measured runs. A light dotted connector labelled **Measurement gap** keeps these runs visually connected; hovering within the gap explicitly says that no measured values are available. These connectors never create observations; the forecast uses cumulative window consumption regardless of gaps. Refills are not connected. A reset starts a new cycle. Disabling/signing out or explicitly switching an account clears its history. Cached responses retain their original observation time. When the provider supplies a stable account identifier, a one-way fingerprint separates accounts even across relaunches. No credentials, raw account identifiers, prompts or task contents are stored in chart history.

## Background energy use

Activity monitoring reads rollout files backwards in 64 KiB chunks and stops at the newest Codex lifecycle event. It avoids decoding and parsing the whole conversation on every two-second poll. Each profile caches the last result until file metadata changes; stale rollouts are not opened. Replacement, truncation and appended events invalidate the cache. Child-item completion and aborted-turn handling retain their existing semantics.

Full-screen detection is cached across mouse movements and invalidated by app, Space and screen changes. A one-second fallback timer (with scheduling tolerance) covers missed notifications and parked pointers. Mouse entry and hover still use the native event monitors immediately. These changes reduce unnecessary CPU work; macOS's energy classification is not an instantaneous validation signal.
